...
# z_scale = (D-1)/2 exactly reproduces the OLD index-normalized convention
# (pz = pos_z*z_scale + (D-1)*0.5 == (pos_z+1)*0.5*(D-1)), so every square grid
# in this file uses that value — these tests exercise splat mechanics, not the
# native-z physical-coordinate convention (that's test_canonical_invariants.py /
# tools/gate_native_z_identity.py).
...
def test_splat_single_point_at_origin():
    ...
    grid = (5, 5, 5)
    vol, cov = splat_to_volume(pos, intensity, grid, z_scale=2.0)  # (D-1)/2 = 2.0
...
def test_splat_sample_roundtrip_isolated_point():
    ...
    z_scale = 3.5  # (8-1)/2
    vol, _ = splat_to_volume(pos, intensity, (8, 8, 8), z_scale)
    sampled = sample_volume(vol, pos, z_scale)
...
def test_splat_disagreeing_points_average():
    ...
    z_scale = 2.0  # (5-1)/2
    vol, _ = splat_to_volume(pos, intensity, (5, 5, 5), z_scale)
    sampled = sample_volume(vol, pos, z_scale)
...
def test_splat_out_of_bounds_contributes_zero():
    ...
    intensity = torch.tensor([[1.0]])
    vol, cov = splat_to_volume(pos, intensity, (4, 4, 4), z_scale=1.5)  # (4-1)/2
...
def test_splat_batched():
    ...
    vol, cov = splat_to_volume(pos, intensity, (6, 6, 6), z_scale=2.5)  # (6-1)/2
...
def test_splat_weight_gating():
    ...
    z_scale = 2.0  # (5-1)/2
    vol, cov = splat_to_volume(pos, intensity, (5, 5, 5), z_scale, weight=weight)
    sampled = sample_volume(vol, pos[:, :1, :], z_scale)  # sample at first point only
...
def test_splat_outputs_fp32_under_bf16_input():
    ...
    vol, cov = splat_to_volume(pos, intensity, (4, 4, 4), z_scale=1.5, weight=weight)
...
def test_splat_bf16_input_matches_fp32_reference():
    ...
    z_scale = 2.5  # (6-1)/2
    vol_fp32, _ = splat_to_volume(pos, intensity, (6, 6, 6), z_scale)
    vol_bf16, _ = splat_to_volume(pos.bfloat16(), intensity.bfloat16(), (6, 6, 6), z_scale)
