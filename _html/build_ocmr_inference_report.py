...
A("<div class='caveat'><b>\"Cleaner\" ≠ better reconstruction (see <span class='mono'>docs/08</span>, reconciled "
  "with <span class='mono'>docs/10</span>/<span class='mono'>11</span>).</b> "
  "These OCMR volumes look cleaner than the model's in-distribution CMRxRecon val visuals, but that is "
  "<b>not</b> superior generalization. The blur is <b>mostly the splat renderer</b> (~75%, <span class='mono'>"
  "docs/10</span>) — common to both val and OCMR, and present even on perfectly clean inputs (0.74× GT). "
  "val looks worse for two reasons: (1) <b>resolution</b> — OCMR inputs are <b>2.4× smoother</b> "
  "(2.16–2.84&nbsp;mm → upsampled), so the splat's fixed detail-loss is invisible on OCMR but smears "
  "<i>visible</i> structure on high-res CMRxRecon; (2) <b>breathing</b> adds val's <i>extra</i> ~25% blur + "
  "dark spots (the resp sim shifts val inputs 16±8&nbsp;mm; toggling it OFF recovers <b>+2.31&nbsp;dB motion "
  "PSNR</b>). A matched, identical-render comparison shows breathing-OFF val is actually the "
  "<b>sharpest/most-detailed</b> of the three; OCMR is clean-but-soft. Black spots are minor at full input "
  "and refiner-solved (<span class='mono'>docs/11</span>). There is no OCMR reference, so OCMR cannot be "
  "PSNR-scored. Full analysis: <span class='mono'>docs/08_ood_clean_paradox.md</span>.</div>")

A("<div class='caveat'><b>Does OCMR really have less breathing? — measured (<span class='mono'>tools/"
  "measure_ocmr_breathing.py</span>).</b> OCMR <i>is</i> free-breathing, so it carries <b>real</b> "
  "respiratory motion — it is not breathing-free. Measured by per-slice frame registration on a "
  "heart-centred crop (subpixel phase-correlation, <b>validated to 0.0&nbsp;px</b> on known shifts), with "
  "the slow respiratory drift separated from the fast cardiac oscillation by frequency: <b>in-plane</b> "
  "respiratory excursion is <b>0.1–0.9&nbsp;mm (median) for the 9 patients</b> and <b>~3&nbsp;mm (up to "
  "11.5&nbsp;mm) for the volunteer us_0084</b>. In-plane is a <b>lower bound</b> on true SI (SAX is tilted "
  "~20–45° off SI → SI ≈ in-plane ÷ sin(tilt), ×1.4–3): estimated SI ≈ <b>0.3–7&nbsp;mm (patients)</b>, "
  "<b>~4–9&nbsp;mm (volunteer)</b>, vs the sim's <b>16±8&nbsp;mm</b>. So real OCMR breathing is "
  "<b>3–50× smaller than the sim for patients</b> (comparable for the volunteer), <i>and</i> it is a "
  "<b>coherent bulk SI slide</b> where the sim is <b>per-slice incoherent</b> (independent random shift + "
  "30° jitter) — at equal amplitude, incoherent scatter distorts a stacked volume far more. <i>Caveat: SI "
  "is extrapolated from the measured in-plane via tilt, not directly measured; the through-plane component "
  "is not observable from a single slice's 2-D cine. Details: <span class='mono'>docs/08 §3.4</span>.</i></div>")
