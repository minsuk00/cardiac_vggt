...
> **All methods timed (2026-09-24, §6), per af12 subject:** Dangi 0.13 s, **VGGT 1.96 s**, CiNeVol
> 70.1 s, NeSVoR 323 s (4× L40S each); SVRTK 167 s, NiftyMIC 233 s, Fetal CMR 4D 517 s (16-CPU Xeon
> Gold 6154). NeSVoR's headline is the 126 subjects on gl1706; its other 54 ran 1.57× slower on
> gl1708 (same GPUs, cause unmeasured).
...
## 8. Plan for the other GPU methods (same 4 GPUs, one patient at a time, §3 span) — implemented (§6b); Dangi, CiNeVol and NeSVoR cohorts all done (§6)
