...
| **Fetal CMR 4D†** (van Amerom) | `vanamerom2019fetal` | upstream `mriphysics/fetal_cmr_4d` + SVRTK `reconstructCardiac` (MATLAB); our LV-area self-gate (docs/35) | `-iterations 4 -rec_iterations 10 -rec_iterations_last 20 -resolution 1.25 -numcardphase 25` | adult HR band; no k-t SENSE; `cardsync_interslice` omitted | UNVERIFIED | **NOT RUN on v2** (MIITT V1 pilot only; single-orientation gating failed, docs/34) |
...
| CiNeVol run-vs-cite; Fetal CMR 4D on v2 | † rows or drop | undecided |
