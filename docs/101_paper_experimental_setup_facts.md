...
| **Segmenter** | nnU-Net v1 (1.7.1) **Task114 (M&Ms) `3d_fullres`, trainer `nnUNetTrainerV2_MMS`**, isolated `nnunet` env; applied to the scored (registered / PSF'd / gauged) cines; GT segmented once per subject and cached **per segmenter config** — `seg_gt/` is 2d, `seg_gt_<cfg>/` otherwise. **Weights:** the authors' official pretrained release, Zenodo record 4288464 (`https://zenodo.org/record/4288464`, `Task114_heart_MNMs.zip`, md5 verified, docs/15), five folds, no fine-tuning; labels 1 = LV, 2 = MYO, 3 = RV. The zip ships **both** `2d` and `3d_fullres` under the same trainer and plans and both are installed under `$RESULTS_FOLDER/nnUNet/`; an earlier revision of this row said "2d config only", which described what was then in use rather than the release. **Cite:** Isensee et al. 2021 Nature Methods (framework, doi 10.1038/s41592-020-01008-z) + Full et al. STACOM 2020 (M&Ms winner, the model) | `run_seg.sh` (`SEG_CFG`, default `3d_fullres`, stamped to `<seg_dir>/nnunet_config`); `ef_dice.py` `SEG_CONFIG`, `_gt_seg_cached`, and the cross-segmenter guard in `score()`; nnunet `download_pretrained_model.py` registry |
| **Segmenter — the one call that stays 2d** | `fetal4d_gate.py`'s `nnUNet_predict -m 2d` is **not** a metric: it is Fetal CMR 4D's **own gating front end**, segmenting the *input* frames to find each slice's ED anchor. Moving it to 3d would change the baseline's reconstructions — i.e. change the method under comparison — and arguably it should stay 2d regardless, since it stands in for what the method could do at acquisition time. **Do not switch it together with the scoring segmenter.** | `fetal4d_gate.py:165` |
