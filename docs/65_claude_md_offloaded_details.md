...
> - **Every 5 val epochs (`filmstrip_every_n_val_epochs`):** one 12-frame cardiac-cycle GIF per selected source/vendor subject under `media_filmstrip/`. ED/ES and augmentation panels use `visual_panels_every_n_val_epochs` (default 3).
...
> 0. **Preprocess (cached, one-time per subject; native-z, docs/58 + docs/74).** monai `PersistentDataset` resamples all 12 phase NIfTIs' **in-plane** axes to `1.4` mm and crops/zero-pads to `256×256` (geometric center) — **Z is never resampled**: `Spacingd(pixdim=(1.4,1.4,0.0))` and `ResizeWithPadOrCropd(spatial_size=(256,256,-1))` keep each subject's own native slice pitch (`dz`, recorded by a `RecordSpacingD` transform) and native slice count (`D`), which both vary per subject (`dz` 5–12mm, `D` 5–21 across the pooled cohort; why z must stay native: docs/58). Normalizes intensity against phase_00's configured `intensity_percentiles` (default 0.5/99.9; computed over non-zero FOV voxels, excluding zero-padding), and stacks into one `(T=12, 256, 256, D)` float16 tensor + a `(256,256,D)` content mask (1=native FOV, 0=zero-pad in X/Y only — Z is never padded) + `dz_mm` (that subject's spacing). The percentile pair is part of the cache signature, so an override routes to a fresh subdirectory under `/tmp/vggt-mri_${USER}_monai_cache/`. Pipeline + custom transforms live in `training/data/preprocess.py`.
