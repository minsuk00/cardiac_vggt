#!/usr/bin/env bash
# Segment the per-phase pred/GT volumes dumped by eval/run_cmrxrecon.py --dump-volumes
# with the M&Ms nnU-Net (Task114, 2d) in the isolated `nnunet` env (docs/15), then the
# EF/Dice analysis is done by eval/seg_metrics_cmrxrecon.py.
#
...
#
# Usage:
#   bash eval/seg_cmrxrecon.sh <dump_dir> <seg_out_dir>
set -euo pipefail
