#!/usr/bin/env bash
# PROBE COPY of evaluation/src/engine/run_fetal4d.sh for tools/fetal4d_nframes_probe.py. Three
# deliberate differences, nothing else touched: (1) the subject dir comes from $PROBE_SD (the
# original hardcodes scratch/eval, this runs under temp/); (2) the engine is fitted on the gate's
# stack4d/cardphase.txt (first N frames) but READ OUT at $GATE/cardphase_readout.txt (all 24 GT
# frames, the same ramp extended), so every N is scored against the same targets; (3) the stamp
# records n_frames_fit. Keep in sync with the original if it changes.
#
...
SD="${PROBE_SD:?PROBE_SD must name the probe subject dir (tools/fetal4d_nframes_probe.py)}"
[ -f "$SD/manifest.json" ] || { echo "no manifest at $SD"; exit 2; }
...
if [ -f "$WD/cine.nii.gz" ] && gzip -t "$WD/cine.nii.gz" 2>/dev/null; then
  read -r N_OK GT_ED < <("$SVR_PY" - "$WD/cine.nii.gz" "$OUT" "$NCP" "$SEG_GT" "$READOUT" "$GATE/cardphase_readout.txt" "$REF_PLANE" "$NF" <<'EOF'
...
if [ "${N_OK:-0}" -eq "$T" ]; then
  printf '{"engine": "fetal_cmr_4d", "input_stack": "rolled", "thickness_mm": %s, "resolution_mm": %s, "iterations": %s, "rec_iterations": "%s/%s", "numcardphase": %s, "n_frames": %s, "n_frames_fit": %s, "rr_nominal_s": %s, "robust_statistics": "%s", "container_id": "%s", "cardphase_count_token": %s, "readout": "%s", "readout_arg": %s, "gate_arm": "%s"}\n' \
    "$THICK" "$RES" "$ITERS" "$NSR" "$NSRLAST" "$NCP" "$NF" "$((NFRAME / NSLICE))" "$MEANRR" "$([ "$ROBUST" = 1 ] && echo on || echo off)" "$(container_id "$SIF")" "$CPCOUNT" "$READOUT" "$GT_ED" "${GATE_ARM:-fetal_cmr_4d}" > "$OUT/stamp.json"
  if [ "$READOUT" = gt_ed_roll ]; then
