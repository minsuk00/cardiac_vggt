...
cardiac 0.827 ED / 0.746 ES, full 0.732 / 0.664; arms score 0.87-0.89 / 0.80-0.84.
...
def val_subject_dirs():
    """(dataset, subject, subject_dir, manifest) for every val-split subject bundle on disk."""
    for man_p in sorted(glob.glob(str(paths.VOLUMES / "*" / "out" / "*" / "manifest.json"))):
        ...
        yield ds, subj, sd, man


def load_subject_seg_dvf(sd):
    """(seg (T,D,H,W) label volume, ed_dvf npz) for one subject dir, or None if the required
    heart_seg.nii.gz / any arm's ed_dvf.npz are missing (not yet scored by any arm)."""
...
def identity_floor_row(ds, subj, man, seg, dvf):
    """One subject's cardiac/full LV Dice floors at ED and ES, or None if the LV seg is empty
    at every phase (unscorable)."""
    T, D, H, W = seg.shape
...
def score_all_subjects():
    """(per_subject rows, n_skipped) over every val-split subject with a seg + phase draw."""
    per_subject, missing = [], 0
    for ds, subj, sd, man in val_subject_dirs():
        loaded = load_subject_seg_dvf(sd)
        if loaded is None:
            missing += 1
            continue
        row = identity_floor_row(ds, subj, man, *loaded)
        if row is None:
            missing += 1
            continue
...
def aggregate_by_dataset(per_subject):
    agg = {}
...
def write_report(out_path, per_subject, agg, missing):
    out = {"meta": {
...
def main():
    ...
    per_subject, missing = score_all_subjects()
    agg = aggregate_by_dataset(per_subject)
    write_report(out_path, per_subject, agg, missing)
