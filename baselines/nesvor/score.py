"""Score a NeSVoR reconstruction against our canonical V_gt.

NeSVoR outputs an isotropic volume at its own resolution/grid (--output-resolution
defaults to 0.8mm, isotropic only -- same limitation as NiftyMIC). We resample it
onto the exact canonical (12, 256, 256) @ (1.4, 1.4, 12.0) mm grid the export
stack came from (== V_gt's grid), then compute the same MAE/PSNR
baselines/niftymic/score.py uses, for a direct side-by-side.

GT is read from baselines/niftymic/'s CLEAN export (scratch/niftymic/data/<tag>_stack.nii.gz)
regardless of which stack variant NeSVoR was actually fed -- matches the trainer's own
contract (target stays at the unshifted reference; only the baseline's INPUT is
corrupted, see export_resp_stack.py). Set NESVOR_RECON_DIR to score a non-default recon
...
DATA_DIR = "/home/minsukc/vggt/scratch/niftymic/data"  # GT source, always the CLEAN stack
RECON_DIR = os.environ.get("NESVOR_RECON_DIR", "/home/minsukc/vggt/scratch/nesvor/recon")
OUT_DIR = "/home/minsukc/vggt/result/nesvor"
...
def _calibrate_intensity(pred, gt):
    ...
    Same rationale as niftymic/score.py: neither classical Tikhonov SRR nor
    NeSVoR's INR solve preserve absolute input intensity scale -- standard in
    the SVR literature to calibrate before scoring PSNR/SSIM.
    """
    anat = gt > 1e-3
...
def score_one(tag):
    ...
    # V_gt == the exported stack itself (same convention as niftymic/score.py):
    # the clean per-phase canonical volume IS our ground truth.
    gt_img = nib.load(stack_path)
    gt_xyz = np.asarray(gt_img.dataobj, dtype=np.float32)

    recon_img = nib.load(recon_path)
    # Resample NeSVoR's (isotropic, its own grid) output onto V_gt's exact
    ...
    print(f"     NeSVoR RAW         MAE_full={m_raw['mae_full']:.4f}  PSNR_full={m_raw['psnr_full']:6.2f} dB  "
          f"MAE_anat={m_raw['mae_anat']:.4f}  PSNR_anat={m_raw['psnr_anat']:6.2f} dB")
    print(f"     NeSVoR CALIBRATED  MAE_full={m_cal['mae_full']:.4f}  PSNR_full={m_cal['psnr_full']:6.2f} dB  "
