...
default.hrRange         = [45,110];  % heart rate range (bpm) -- MIITT: ADULT (was fetal [105,180])
...
%% Load Real-Time Data
%  MIITT PATCH: MIITT is already reconstructed, so there is no ktrecon xtRcn.mat.
%  Load the real-time magnitude frames straight from the NIfTI instead. The NIfTI
%  is (x, y, loc, frame) -- exactly the layout the original derives from xtRcn --
%  so the per-slice Y{iLoc} = (x,y,1,frame) contract is unchanged. estimate_
%  heartrate_xf operates on magnitude (abs(fft) over time), so magnitude in place
%  of the complex xtRcn is fine (the cardiac peak is present in magnitude).

for iStk = 1:nStack

    % Load Reconstructed Dynamic MRI from NIfTI
    R = load_untouch_nii( S(iStk).rltAbFile );
    Y = double( R.img );   % (x, y, loc, frame)
...
if exist( 'hFig', 'var' ) && isvalid( hFig )   % MIITT: guard stale/closed handle when verbose=false
