...
2. **Aggregator.** DINOv2 patch_embed + 24× alternating frame/global attention. Per-slot special token = sinusoidal embeddings: `z_embedder(z_norm)` (linear, `z_norm = z/(D-1)*2-1`, D=12) always on; reference default adds the two-token `camera_token` (slot 0 = target-phase anchor), legacy path adds `t_embedder`+`target_t_embedder` instead (see Project + Key knobs). Frozen vs aggft per the freeze-pattern note.
