...
if [ "${SLURM_RESTART_COUNT:-0}" -gt 0 ]; then
    ...
    if [ ! -z "$RESUME_FROM" ]; then
        ...
        EXTRA_OVERRIDES="max_epochs=100 max_img_per_gpu=12 one_frame_per_slice=true checkpoint.save_freq=50 loss.volume.gather_weight=0.0 logging.wandb_writer.tags=[mri_volume_diffusion,multiphase,1frame_series,no_gather]"
        OVERRIDES="exp_name=${EXP_NAME} checkpoint.resume_checkpoint_path=${CKPT_ONLY} ${EXTRA_OVERRIDES}"
