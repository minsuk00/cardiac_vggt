...
RESUME_FROM="/home/minsukc/vggt/scratch/logs/216003592_mri_volume_diffusion_oneframe_aug_moderate_dynamic_axial_pooled1343"
...
ABLATION_OVERRIDES="max_epochs=100 one_frame_per_slice=true checkpoint.save_freq=50 loss.volume.gather_weight=0.5 data.augmentation.enable=true data.augmentation.tier=moderate logging.wandb_writer.tags=[mri_volume_diffusion,multiphase,1frame_series,aug_moderate]"

# --- Self-Submission Logic ---
if [ -z "$SLURM_JOB_ID" ]; then
    TIMESTAMP=$(date +%Y%m%d_%H%M%S)
    JOB_NAME="vggt_1f_aug"
...
if [ "${SLURM_RESTART_COUNT:-0}" -gt 0 ]; then
    ...
    if [ ! -z "$RESUME_FROM" ]; then
        ...
        EXP_NAME="${REV_TS}_mri_volume_diffusion_oneframe_aug_moderate_dynamic_axial_pooled1337"
        EXTRA_OVERRIDES="${ABLATION_OVERRIDES}"
        OVERRIDES="exp_name=${EXP_NAME} checkpoint.resume_checkpoint_path=${CKPT_ONLY} ${EXTRA_OVERRIDES}"
