...
# ============================================================================================
# TWOPHASE-DIFF SWEEP ARM w=0.5, 2026-08-24 — runs from the vggt-twophase WORKTREE (branch
# arm/twophase-diff, the two-phase difference loss). Recipe byte-identical to the awrobewn
# baseline (train_pooled1337_dpt_augaggressive_224.sh / run 213340611): img 224, aggressive
# aug, 300 epochs, LR 5e-5 (three knobs), heart_weight 0.5 — the changes are ARM_OVERRIDES
# below: diffusion 0 (the confirmed no-reg ranking win, r .792→.837) + diff_weight (the
# between-phase swing term, ~2× forward cost). Judge vs no-reg ep300: SV-ratio (0.712),
# EF bias (−12.6), per-patient r (0.837) — win = SV↑ WITH r held; SV↑+r↓ = cohort-mean
# cheat, reject. Re-score decisive ckpts with nnU-Net Task114 via temp/dice_ef_sweep/.
# ============================================================================================
REPO="/home/minsukc/vggt-twophase"
CONFIG="default"

# --- THE ARM VARIABLE ---
ARM_OVERRIDES="loss.volume.diffusion_weight=0.0 loss.volume.diff_weight=0.5"
VARIANT_TAG="twophase05"
...
if [ -z "$SLURM_JOB_ID" ]; then
    ...
    JOB_NAME="vggt_twophase_${VARIANT_TAG}"
