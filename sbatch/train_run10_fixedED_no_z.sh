...
# =============================================================================
# 200k-step head-only training run, fixed ED (t=0) target — ABLATION: z (slice
# position) embedding DISABLED. Fresh run (loads VGGT-1B base on cold start, trains
# only point_head — aggregator + patch_embed frozen) with SLURM auto-requeue.
#
# This is run1 (fixedED head-only) with use_z_pose_embedding=false: the aggregator
# no longer adds the linear z Fourier code, so the model cannot tell which canonical
# z-plane each input slice came from. t embedding stays ON. Tests whether the z code
# matters for ED reconstruction.
#
...
CONFIG="mri_volume"
NGPU=1
EXP_TAG="mri_volume_fixedED_no_z"            # → exp_name = <rev_ts>_${EXP_TAG}
RUN_OVERRIDES="max_epochs=200 t_target_fixed=0 use_z_pose_embedding=false logging.wandb_writer.tags=[mri_volume,t0,headonly,no_z_embed]"   # fixed ED (t=0) target, head-only, z embedding off
MASTER_PORT=29540
# -------------------------------------------------------------------------------
