...
# =============================================================================
# REFINER run A2 (FROZEN + SSIM, NEW SEED) — same config as train_refiner_frozen_ssim.sh
# BUT seeds from the ACTUAL OLD checkpoint_last.pt (218747856), not the earlier
# t59w6nqy_resp_no_t_weights_only.pt snapshot.
#
# Motivation (docs/18): the original bpblrai2 ckpt (218246076) warm-started from
# scratch/base_weights/t59w6nqy_resp_no_t_weights_only.pt, which is the SAME run as
# 218747856 but exported ~27h BEFORE checkpoint_last.pt was saved (per-tensor diff
# 1.51e-2). So "OLD V_canon" (checkpoint_last) ≠ "bpblrai2 V_canon" (earlier snapshot)
# is a snapshot mismatch, not a sanity-check failure. This run seeds from the latest
# OLD weights, so OLD V_canon = this-run V_canon exactly → the SSIM-refiner-effect
# isolation becomes apples-to-apples.
# =============================================================================

# ---- PER-RUN CONFIG ----
CONFIG="mri_volume"
NGPU=1
EXP_TAG="mri_refiner_frozen_ssim_newseed"
SEED_FULL="/home/minsukc/vggt/scratch/logs/218747856_mri_volume_resp_allphases_aggft_z_no_t/ckpts/checkpoint_last.pt"
SEED_WEIGHTS_ONLY="/home/minsukc/vggt/scratch/base_weights/218747856_last_weights_only.pt"
RUN_OVERRIDES="max_epochs=200 enable_refiner=true refiner_use_coverage=true data.augmentation.respiratory.enable=true use_z_pose_embedding=true use_t_pose_embedding=false use_target_t_pose_embedding=true optim.frozen_module_names=[*patch_embed*,*camera_token*,*aggregator*,*point_head*] distributed.find_unused_parameters=true loss.volume.refiner_lambda=1.0 loss.volume.refiner_ssim_weight=0.1 checkpoint.resume_checkpoint_path=${SEED_WEIGHTS_ONLY} logging.wandb_writer.tags=[mri_volume,allphases,multiphase,resp,no_t,refiner,frozen,ssim,newseed]"
MASTER_PORT=29568
# ------------------------
