...
# =============================================================================
# 400k-step training run — one of a 4-run series. All four scripts share an
# IDENTICAL body; only the "PER-RUN CONFIG" block below differs. Each is a fresh
# run (loads VGGT-1B base on cold start, then trains) and auto-requeues across the
# 48h walltime cap, resuming from its own ckpts/checkpoint_last.pt.
#
# 400k optimizer steps = 400 epochs  (limit_train_batches=1000, accum_steps=1, 1 GPU).
# =============================================================================

# ---- PER-RUN CONFIG (the only block that differs across the 4 run scripts) ----
CONFIG="mri_volume"
NGPU=1
EXP_TAG="mri_volume_mixed_aggft"             # → exp_name = <rev_ts>_${EXP_TAG}
# run 4: mixed (multi-phase) target + finetune aggregator (DINO patch_embed stays frozen).
# Unfreezing the aggregator exposes params with no grad in the point-only forward
# (camera/register tokens, disabled heads) → DDP needs find_unused_parameters=true even
# on 1 GPU. ~637M trainable, ~27GB on the A40, ~2.8x slower than head-only. No augmentation.
RUN_OVERRIDES="max_epochs=400 optim.frozen_module_names=[*patch_embed*] distributed.find_unused_parameters=true logging.wandb_writer.tags=[mri_volume,multiphase,aggft]"
MASTER_PORT=29534
# -------------------------------------------------------------------------------
