...
# =============================================================================
# 200k-step head-only training run, fixed ED (t=0) target — ABLATION: t (cardiac
# phase) embedding DISABLED. Fresh run (loads VGGT-1B base on cold start, trains
# only point_head — aggregator + patch_embed frozen) with SLURM auto-requeue.
#
# This is run1 (fixedED head-only) with use_t_pose_embedding=false: the aggregator
# no longer adds the cyclic t Fourier code, so the model cannot tell which cardiac
# phase each input slice came from. z embedding stays ON. Tests whether the t code
# matters when the target is always ED. Input slots still span phases (dynamic
# sampling), so this measures phase-agnostic ED reconstruction.
#
...
CONFIG="mri_volume"
NGPU=1
EXP_TAG="mri_volume_fixedED_no_t"            # → exp_name = <rev_ts>_${EXP_TAG}
RUN_OVERRIDES="max_epochs=200 t_target_fixed=0 use_t_pose_embedding=false logging.wandb_writer.tags=[mri_volume,t0,headonly,no_t_embed]"   # fixed ED (t=0) target, head-only, t embedding off
MASTER_PORT=29539
# -------------------------------------------------------------------------------
