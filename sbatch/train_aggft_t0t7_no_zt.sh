...
CONFIG="mri_volume"
NGPU=1
EXP_TAG="mri_volume_t0t7_aggft_no_zt"     # → exp_name = <rev_ts>_${EXP_TAG} (fresh only; ignored when RESUME_FROM set)
RUN_OVERRIDES="max_epochs=200 t_target_phases=[0,7] use_z_pose_embedding=false use_t_pose_embedding=false optim.frozen_module_names=[*patch_embed*] distributed.find_unused_parameters=true logging.wandb_writer.tags=[mri_volume,t0t7,multiphase,aggft,no_zt]"   # ABLATION: only target_t (z + t_self embeddings OFF); aggft {0,7}
MASTER_PORT=29542
# -------------------------------------------------------------------------------
