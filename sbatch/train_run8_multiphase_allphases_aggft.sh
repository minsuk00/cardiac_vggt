...
CONFIG="mri_volume"
NGPU=1
EXP_TAG="mri_volume_allphases_aggft"      # → exp_name = <rev_ts>_${EXP_TAG}
RUN_OVERRIDES="max_epochs=200 optim.frozen_module_names=[*patch_embed*] distributed.find_unused_parameters=true logging.wandb_writer.tags=[mri_volume,allphases,multiphase,aggft]"   # multiphase all T phases, head + aggregator (DINO patch_embed frozen)
MASTER_PORT=29538
# -------------------------------------------------------------------------------
