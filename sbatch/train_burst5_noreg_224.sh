...
# --- Configuration ---
# ============================================================================================
# BURST-SAMPLER ARM (branch arm/burst5, worktree /home/minsukc/vggt-burst), 2026-08-25.
# Recipe = the noreg224 arm (docs/90 job 58421975: awrobewn augaggr224 + diffusion_weight=0)
# with ONE more change: `burst_k=5` — every in-bbox z-plane contributes a burst of 5
# temporally SEQUENTIAL frames (t0 random per plane, mod-T wrap; slot 0 stays the
# target-phase reference), simulating a short real-time acquisition burst per slice.
# Breathing coherence within a burst comes from the default respiratory group_by_burst
# (one breath phase per z-plane). S = 1 + 5*D (max 91 on train/val); img_nums AND
# num_slices raised to 106 (both budget checks: dataloader + trainer_viz).
# gradient_checkpointing TRUE (unlike the k=1 224 arms): ~5x the tokens. Smoke-measured
# on A40: ~21 GB, ~2.8 s/train step.
# Baseline for comparison = the noreg224 arm itself (k=1, same everything else).
# ============================================================================================
#
...
#
# NO-REG (docs/90): diffusion_weight=0 — objective is full-volume L1 + heart-L1(0.5) +
# gather(0.5). Respiration ON via default.yaml (docs/05), tier=aggressive affine aug.
#
# DEVIATIONS FROM default.yaml, applied via RECIPE_OVERRIDES below:
#   max_epochs 200 -> 300
#   burst_k 1 -> 5, num_slices/img_nums 20 -> 106
#   loss.volume.diffusion_weight 1000 -> 0
#   gradient checkpointing true (kept ON — S up to 91)
...
data.train.common_config.img_nums=[106,106] \
data.val.common_config.img_nums=[106,106] \
data.train.dataset.dataset_configs.0.num_slices=106 \
data.val.dataset.dataset_configs.0.num_slices=106 \
...
cd /home/minsukc/vggt-burst   # the arm/burst5 worktree — burst_k exists only on this branch
