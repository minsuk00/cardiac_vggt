...
# =============================================================================
# 200k-step head-only training run, fixed ED (t=0) target — ABLATION: BOTH z (slice
# position) and t (cardiac phase) embeddings DISABLED. Fresh run (loads VGGT-1B base
# on cold start, trains only point_head — aggregator + patch_embed frozen) with
# SLURM auto-requeue.
#
# This is run1 (fixedED head-only) with use_z_pose_embedding=false AND
# use_t_pose_embedding=false: with neither pose code, the aggregator falls back to
# the original (frozen) camera_token — every input slice is positionally
# indistinguishable. This is the no-pose-information floor for ED reconstruction.
#
...
CONFIG="mri_volume"
NGPU=1
EXP_TAG="mri_volume_fixedED_no_zt"           # → exp_name = <rev_ts>_${EXP_TAG}
RUN_OVERRIDES="max_epochs=200 t_target_fixed=0 use_z_pose_embedding=false use_t_pose_embedding=false logging.wandb_writer.tags=[mri_volume,t0,headonly,no_zt_embed]"   # fixed ED (t=0) target, head-only, both z+t embeddings off
MASTER_PORT=29541
# -------------------------------------------------------------------------------
