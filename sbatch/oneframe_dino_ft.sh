...
#
# WARM-START: weights-only from 4wok (CKPT_ONLY=scratch/checkpoints/4wok_weights_only.pt,
...
RESUME_FROM="/home/minsukc/vggt/scratch/logs/216002704_mri_volume_diffusion_oneframe_dino_ft_lr2e5_dynamic_axial_Cine_combined"
...
ABLATION_OVERRIDES="max_epochs=100 max_img_per_gpu=12 one_frame_per_slice=true checkpoint.save_freq=50 loss.volume.gather_weight=0.5 optim.frozen_module_names=[] optim.optimizer.lr=2e-5 optim.options.lr.0.scheduler.schedulers.0.end_value=2e-5 optim.options.lr.0.scheduler.schedulers.1.start_value=2e-5 logging.wandb_writer.tags=[mri_volume_diffusion,multiphase,1frame_series,dino_ft,lr2e5]"

# --- Self-Submission Logic ---
if [ -z "$SLURM_JOB_ID" ]; then
    TIMESTAMP=$(date +%Y%m%d_%H%M%S)
    JOB_NAME="vggt_1f_dino"
...
if [ "${SLURM_RESTART_COUNT:-0}" -gt 0 ]; then
    ...
    if [ ! -z "$RESUME_FROM" ]; then
        ...
        EXP_NAME="${REV_TS}_mri_volume_diffusion_oneframe_dino_ft_lr2e5_dynamic_axial_Cine_combined"
        EXTRA_OVERRIDES="${ABLATION_OVERRIDES}"
        OVERRIDES="exp_name=${EXP_NAME} checkpoint.resume_checkpoint_path=${CKPT_ONLY} ${EXTRA_OVERRIDES}"
        echo "ONEFRAME dino_ft (unfreeze patch_embed, lr 2e-5): exp_name=${EXP_NAME}"
    else
