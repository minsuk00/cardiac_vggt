...
if [ "${SLURM_RESTART_COUNT:-0}" -gt 0 ]; then
    ...
    if [ ! -z "$RESUME_FROM" ]; then
        ...
        EXTRA_OVERRIDES="max_epochs=100 max_img_per_gpu=12 one_frame_per_slice=true checkpoint.save_freq=50 loss.volume.gather_weight=0.5 optim.frozen_module_names=[] optim.optimizer.lr=2e-5 optim.options.lr.0.scheduler.schedulers.0.end_value=2e-5 optim.options.lr.0.scheduler.schedulers.1.start_value=2e-5 logging.wandb_writer.tags=[mri_volume_diffusion,multiphase,1frame_series,dino_ft,lr2e5]"
        OVERRIDES="exp_name=${EXP_NAME} checkpoint.resume_checkpoint_path=${CKPT_ONLY} ${EXTRA_OVERRIDES}"
