...
PEAK_LR="5e-5"
# --- OLD-DENSITY 518 on HEAD, 2026-09-06: identical to train_pooled1337_dpt_augaggressive_518native.sh
# except loss.volume.splat_res=518, which makes the training splat scatter 518^2 up-sampled points per
# slice (the pre-docs/73 model-res path the old 518 arm jyctv2mm trained against) while everything else
# is HEAD. If THIS run tracks old-518 while the native replicate (211475121) tracks 224/336, the splat
# density is the cause; if it tracks the native replicate, the cause is elsewhere in the code drift.
# table (docs/86 sweep). Old 518 arm (jyctv2mm, job 213415771) trained at commit 02484cd, BEFORE
# the docs/73 native-splat port (a031ba7), so it splatted the 518^2 up-sampled image; the 224/336
# arms trained after it. This run = same recipe on HEAD (native 256^2 splat). diffusion_weight is
# pinned to 1000 explicitly (default.yaml flipped 1000->0 in 652312b, after all three arms).
# img_size=518 + gradient_checkpointing=true are the default.yaml values (stated for clarity).
...
VARIANT_TAG="augaggr518splat518"
