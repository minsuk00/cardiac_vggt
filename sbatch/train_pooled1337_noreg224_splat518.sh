...
PEAK_LR="5e-5"
# --- NO-REG 224 + DENSE TRAINING SPLAT, 2026-09-06. The no-reg paper recipe (212682843, diffusion=0,
# aggressive aug, gc off) with loss.volume.splat_res=518: field + native slice resampled to 518^2 before
# the training splat (same point density the old 518 arm trained against). Tests whether the old-518
# EF/Dice edge is a splat-density effect that transfers to 224. Eval render is unchanged (native).
...
VARIANT_TAG="noreg224splat518${VARIANT_SUFFIX:-}"
